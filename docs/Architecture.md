...
## Config Interaction
All internal components read from a unified config file (currently `watcher.yaml`; will be renamed).  
...
- Constants at the top pin the inbox/processed roots to `/srv/copyparty/inbox` and `/srv/copyparty/processed`, the repo root to `/srv/repos`, and the runtime config path to `/etc/codex-runner/config.toml`.
- `load_config()` merges `DEFAULT_CONFIG` with an optional TOML file (loaded via `tomllib` or the external `toml` package) so watcher defaults remain predictable when the file is missing or invalid.
...
- The script always reads `/srv/prompt-valet/config/codex-runner.yaml` via `yaml.safe_load()`, merging into `DEFAULT_CONFIG` so tree-builder and watcher defaults stay aligned.
