...
## Config Interaction
All internal components read from a unified YAML config file at `/srv/prompt-valet/config/prompt-valet.yaml`.  
...
- `load_config()` merges `DEFAULT_CONFIG` with an optional YAML file (loaded via `yaml.safe_load()`) so watcher defaults remain predictable when the file is missing or invalid.
...
- The script always reads `/srv/prompt-valet/config/prompt-valet.yaml` via `yaml.safe_load()`, merging into `DEFAULT_CONFIG` so tree-builder and watcher defaults stay aligned.
